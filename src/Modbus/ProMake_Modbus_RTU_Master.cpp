...
ProMake_Modbus_RTU_Master::ProMake_Modbus_RTU_Master(ProMake_RS485 *s)
    : _timeout(100), _s(s)
{
}

ProMake_Modbus_RTU_Master::ProMake_Modbus_RTU_Master()
    : _timeout(100), _s(NULL)
{
}

void ProMake_Modbus_RTU_Master::setTimeoutTimeMs(uint32_t timeout)
{
    _timeout = timeout;
}

bool ProMake_Modbus_RTU_Master::readCoilsRegister(uint8_t id, uint16_t reg)
{
...
bool ProMake_Modbus_RTU_Master::readDiscreteInputsRegister(uint8_t id, uint16_t reg)
{
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), 0x00, 0x01};
    bool val = false;
    uint8_t ret = 0;
...
uint16_t ProMake_Modbus_RTU_Master::readHoldingRegister(uint8_t id, uint16_t reg)
{
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), 0x00, 0x01};
    uint16_t val = 0;
    uint8_t ret = 0;
...
uint16_t ProMake_Modbus_RTU_Master::readInputRegister(uint8_t id, uint16_t reg)
{
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), 0x00, 0x01};
    uint16_t val = 0;
    uint8_t ret = 0;
...
uint8_t ProMake_Modbus_RTU_Master::writeCoilsRegister(uint8_t id, uint16_t reg, bool flag)
{
...
uint8_t ProMake_Modbus_RTU_Master::writeHoldingRegister(uint8_t id, uint16_t reg, uint16_t val)
{
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), (uint8_t)((val >> 8) & 0xFF), (uint8_t)(val & 0xFF)};
    uint8_t ret = 0;
    if (id > 0xF7)
...
uint8_t ProMake_Modbus_RTU_Master::readCoilsRegister(uint8_t id, uint16_t reg, uint16_t regNum, uint8_t *data, uint16_t size)
{
...
uint8_t ProMake_Modbus_RTU_Master::readDiscreteInputsRegister(uint8_t id, uint16_t reg, uint16_t regNum, uint8_t *data, uint16_t size)
{
    uint8_t length = regNum / 8 + ((regNum % 8) ? 1 : 0);
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), (uint8_t)((regNum >> 8) & 0xFF), (uint8_t)(regNum & 0xFF)};
    uint8_t ret = 0;
...
uint8_t ProMake_Modbus_RTU_Master::readHoldingRegister(uint8_t id, uint16_t reg, void *data, uint16_t size)
{
...
uint8_t ProMake_Modbus_RTU_Master::readInputRegister(uint8_t id, uint16_t reg, void *data, uint16_t size)
{
    uint8_t length = size / 2 + ((size % 2) ? 1 : 0);
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), (uint8_t)((length >> 8) & 0xFF), (uint8_t)(length & 0xFF)};
    uint8_t ret = 0;
...
uint8_t ProMake_Modbus_RTU_Master::readHoldingRegister(uint8_t id, uint16_t reg, uint16_t *data, uint16_t regNum)
{
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), (uint8_t)((regNum >> 8) & 0xFF), (uint8_t)(regNum & 0xFF)};
    uint8_t ret = 0;
    if ((id == 0) && (id > 0xF7))
...
uint8_t ProMake_Modbus_RTU_Master::readInputRegister(uint8_t id, uint16_t reg, uint16_t *data, uint16_t regNum)
{
    uint8_t temp[] = {(uint8_t)((reg >> 8) & 0xFF), (uint8_t)(reg & 0xFF), (uint8_t)((regNum >> 8) & 0xFF), (uint8_t)(regNum & 0xFF)};
    uint8_t ret = 0;
    if ((id == 0) && (id > 0xF7))
...
uint8_t ProMake_Modbus_RTU_Master::writeCoilsRegister(uint8_t id, uint16_t reg, uint16_t regNum, uint8_t *data, uint16_t size)
{
    uint16_t length = regNum / 8 + ((regNum % 8) ? 1 : 0);
    if (size < length)
        return (uint8_t)eRTU_EXCEPTION_ILLEGAL_DATA_VALUE;
#if defined(ESP8266)
    uint8_t temp[size + 5];
...
uint8_t ProMake_Modbus_RTU_Master::writeHoldingRegister(uint8_t id, uint16_t reg, void *data, uint16_t size)
{
...
uint8_t ProMake_Modbus_RTU_Master::writeHoldingRegister(uint8_t id, uint16_t reg, uint16_t *data, uint16_t regNum)
{
...
ProMake_Modbus_RTU_Master::pRtuPacketHeader_t ProMake_Modbus_RTU_Master::packed(uint8_t id, eFunctionCommand_t cmd, void *data, uint16_t size)
{
    return packed(id, (uint8_t)cmd, data, size);
}

ProMake_Modbus_RTU_Master::pRtuPacketHeader_t ProMake_Modbus_RTU_Master::packed(uint8_t id, uint8_t cmd, void *data, uint16_t size)
{
...
void ProMake_Modbus_RTU_Master::sendPackage(pRtuPacketHeader_t header)
{
    clearRecvBuffer();
    if (header != NULL)
    {
        _s->noReceive();
        _s->beginTransmission();
...
ProMake_Modbus_RTU_Master::pRtuPacketHeader_t ProMake_Modbus_RTU_Master::recvAndParsePackage(uint8_t id, uint8_t cmd, uint16_t data, uint8_t *error)
{
    if (id > 0xF7)
        return NULL;
    if (id == 0)
...
uint16_t ProMake_Modbus_RTU_Master::calculateCRC(uint8_t *data, uint8_t len)
{
    uint16_t crc = 0xFFFF;
...
void ProMake_Modbus_RTU_Master::clearRecvBuffer()
{
    while (_s->available())
    {
        _s->read();
