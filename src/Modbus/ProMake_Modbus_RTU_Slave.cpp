#include "ProMake_Modbus_RTU_Slave.h"

ProMake_Modbus_RTU_Slave::ProMake_Modbus_RTU_Slave(ProMake_RS485 &serial, uint8_t *buf, uint16_t bufSize, uint32_t responseDelay)
{
    _serial = &serial;
    _buf = buf;
    _bufSize = bufSize;
...
void ProMake_Modbus_RTU_Slave::configureCoils(uint16_t numCoils, BoolRead coilRead, BoolWrite coilWrite)
{
    _numCoils = numCoils;
    _coilRead = coilRead;
...
void ProMake_Modbus_RTU_Slave::configureDiscreteInputs(uint16_t numDiscreteInputs, BoolRead discreteInputRead)
{
    _numDiscreteInputs = numDiscreteInputs;
    _discreteInputRead = discreteInputRead;
}

void ProMake_Modbus_RTU_Slave::configureHoldingRegisters(uint16_t numHoldingRegisters, WordRead holdingRegisterRead, WordWrite holdingRegisterWrite)
{
...
void ProMake_Modbus_RTU_Slave::configureInputRegisters(uint16_t numInputRegisters, WordRead inputRegisterRead)
{
    _numInputRegisters = numInputRegisters;
    _inputRegisterRead = inputRegisterRead;
}

void ProMake_Modbus_RTU_Slave::begin(uint8_t id, uint32_t baud, uint8_t config)
{
    _id = id;
...
void ProMake_Modbus_RTU_Slave::poll()
{
    _serial->receive();
    if (_serial->available() > 0)
    {
        ...
        if (_serial->available() == 0 && (_buf[0] == _id || _buf[0] == 0) && _crc(i - 2) == _bytesToWord(_buf[i - 1], _buf[i - 2]))
        {
            ...
            case 5: /* Write Single Coil */
            {
                ...
                else if (!_coilWrite(address, value))
            ...
            case 6: /* Write Single Holding Register */
            {
                ...
                if (address >= _numHoldingRegisters)
                    _exceptionResponse(2);
                else if (!_holdingRegisterWrite(address, value))
                    _exceptionResponse(4);
                else
                    _write(6);
            }
...
void ProMake_Modbus_RTU_Slave::_processBoolRead(uint16_t numBools, BoolRead boolRead)
{
    uint16_t startAddress = _bytesToWord(_buf[2], _buf[3]);
    uint16_t quantity = _bytesToWord(_buf[4], _buf[5]);
...
void ProMake_Modbus_RTU_Slave::_processWordRead(uint16_t numWords, WordRead wordRead)
{
    uint16_t startAddress = _bytesToWord(_buf[2], _buf[3]);
    uint16_t quantity = _bytesToWord(_buf[4], _buf[5]);
...
void ProMake_Modbus_RTU_Slave::_exceptionResponse(uint8_t code)
{
    _buf[1] |= 0x80;
    _buf[2] = code;
    _write(3);
...
void ProMake_Modbus_RTU_Slave::_write(uint8_t len)
{
...
uint16_t ProMake_Modbus_RTU_Slave::_crc(uint8_t len)
{
    uint16_t crc = 0xFFFF;
...
uint16_t ProMake_Modbus_RTU_Slave::_div8RndUp(uint16_t value)
{
    return (value + 7) >> 3;
}

uint16_t ProMake_Modbus_RTU_Slave::_bytesToWord(uint8_t high, uint8_t low)
{
    return (high << 8) | low;
}
