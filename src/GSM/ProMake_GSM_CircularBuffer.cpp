...
*/
#include "ProMake_GSM_CircularBuffer.h"
#include <Arduino.h>

ProMake_GSM_CircularBuffer::ProMake_GSM_CircularBuffer(ProMake_GSM_CircularBufferManager* mgr)
{
	head=0;
	tail=0;
	cbm=mgr;
...
int ProMake_GSM_CircularBuffer::write(char c)
{
...
char ProMake_GSM_CircularBuffer::read()
{
	char res;
	if(head!=tail)
	{
...
char ProMake_GSM_CircularBuffer::peek(int increment)
{
	char res;
	byte num_aux;
...
void ProMake_GSM_CircularBufferManager::spaceAvailable(){return;};

void ProMake_GSM_CircularBuffer::flush()
{
	head=tail;
}

char* ProMake_GSM_CircularBuffer::nextString()
{
	while(head!=tail)
	{
		head=(head+1) & __BUFFERMASK__;
...
bool ProMake_GSM_CircularBuffer::locate(const char* reference)
{

	return locate(reference, head, tail, 0, 0);
}

bool ProMake_GSM_CircularBuffer::chopUntil(const char* reference, bool movetotheend, bool usehead)
{
	byte from, to;
...
bool ProMake_GSM_CircularBuffer::locate(const char* reference, byte thishead, byte thistail, byte* from, byte* to)
{
	int refcursor=0;
	bool into=false;
	byte b2, binit;
...
bool ProMake_GSM_CircularBuffer::extractSubstring(const char* from, const char* to, char* buffer, int bufsize)
{
	byte t1;
	byte h2;
	byte b;
...
int ProMake_GSM_CircularBuffer::readInt()
{
	int res=0;
	byte c;
	bool anyfound=false;
...
void ProMake_GSM_CircularBuffer::debugBuffer()
{
	byte h1=head;
	byte t1=tail;
	Serial.println();
...
void ProMake_GSM_CircularBuffer::printCharDebug(uint8_t c)
{
	if((c>31)&&(c<127))
		Serial.print((char)c);
	else
...
bool ProMake_GSM_CircularBuffer::retrieveBuffer(char* buffer, int bufsize, int& SizeWritten)
{
	byte b;
	int i;
